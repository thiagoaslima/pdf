var MagazineView = {
    ...
    start: function () {
        if (PDFViewerApplication.sidebarOpen) document.getElementById('sidebarToggle').click();
        ...
        MagazineView.loadTurnJsPages(pages, $('#magazine'), true, true).then(function () {
            ...
            setTimeout(function () {
                ...
                if (MagazineView.currentPage > 1) $("#magazine").turn("page", MagazineView.currentPage);
                ...
                $('.zoom-icon').bind('click', function () {
                    if ($(this).hasClass('zoom-icon-in')) $('#magazineContainer').zoom('zoomIn');
                    else if ($(this).hasClass('zoom-icon-out')) $('#magazineContainer').zoom('zoomOut');
                });
            }, 10);
        });
    },
    showHidePageButtons: function (page) {
        ...
        if (page == 1) $('#magazineContainer .previous-button').hide();
            ...
            $('#magazineContainer .previous-button').show();
        if (page == $('#magazine').turn('pages')) $('#magazineContainer .next-button').hide();
        else
    ...
    loadTurnJsPages: function (pages, magazine, isInit, defer, scale) {
        ...
        for (var i = 0; i < pages.length; i++) {
            PDFViewerApplication.pdfDocument.getPage(pages[i]).then(function (page) {
                ...
                if (MagazineView.currentScale > 1) viewport = page.getViewport(MagazineView.currentScale);
